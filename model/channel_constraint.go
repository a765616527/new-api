...
// A missing id in channelsIDM is kept for routing filters so the downstream
// consistency error is preserved, and dropped for task-plugin identity.
...
func candidatePassesKindFilters(ch *Channel, exists bool, modelName string, kind dto.ChannelFilterKind, filters []dto.ChannelFilter) bool {
	if (kind == dto.FilterRequestPath || kind == dto.FilterImageSizeTier) && !exists {
		return true
	}
	if !exists || ch == nil {
		return false
