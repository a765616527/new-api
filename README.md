...
本仓库基于 [QuantumNous/new-api](https://github.com/QuantumNous/new-api) 持续同步，在保留 New API 原有能力的基础上，增加了 `gpt-image-2` 分辨率路由和计费支持。

### GPT Image 2 分辨率路由

- 客户端继续使用标准 OpenAI 图片协议，请求模型统一填写 `gpt-image-2`。
- 系统按照 `size` 的最长边自动选择 1K、2K 或 4K 档位。
- 最长边不超过 1536 像素时使用 1K；超过 1536 且不超过 2048 像素时使用 2K；超过 2048 像素时使用 4K。
- `size` 为 `auto` 或省略时使用 4K。
- 每个渠道可以为三个档位分别设置不同的上游模型名称；未配置某档位的渠道不会参与该档位的选择和重试。
- JSON 请求体透传和 multipart 图片编辑场景都会正确改写上游模型，同时保留客户端的其他标准请求参数。

### 分辨率分档计费

- 可以为 `gpt-image-2` 分别设置 1K、2K、4K 的每张图片价格。
- 实际费用为：档位单价 × 请求数量 `n` × 分组倍率。
- 某个档位未设置价格时，会使用 `gpt-image-2` 的基础固定价格。
...
1. 在渠道支持模型中加入 `gpt-image-2`。
2. 编辑渠道，在 GPT Image 2 分辨率路由区域填写 1K、2K、4K 对应的上游模型。
3. 在系统模型定价中编辑 `gpt-image-2`，设置基础固定价格和各分辨率档位价格。
