...
本仓库基于 [QuantumNous/new-api](https://github.com/QuantumNous/new-api) 持续同步，在保留 New API 原有能力的基础上，增加了 GPT Image 2 / 2.5 分辨率路由和计费支持。

### GPT Image 2 / 2.5 分辨率路由

- 客户端继续使用标准 OpenAI 图片协议。支持的请求模型：`gpt-image-2`、`gpt-image-2.5-flare`、`gpt-image-2.5-sunburst`。
- Flare 是 2.5 的快速档，Sunburst 是精细档；二者尺寸规则与 Image 2 相同，但模型名必须分开填写，不能写成 `gpt-image-2.5`。
- 系统按照 `size` 的最长边自动选择 1K、2K 或 4K 档位。
- 最长边不超过 1536 像素时使用 1K；超过 1536 且不超过 2048 像素时使用 2K；超过 2048 像素时使用 4K。
- `size` 为 `auto` 或省略时使用 4K。
- 每个渠道可以为每个模型的三个档位分别设置不同的上游模型名称；未配置某档位的渠道不会参与该档位的选择和重试。
- JSON 请求体透传和 multipart 图片编辑场景都会正确改写上游模型，同时保留客户端的其他标准请求参数。

### 分辨率分档计费

- 可以为上述每个模型分别设置 1K、2K、4K 的每张图片价格。
- 实际费用为：档位单价 × 请求数量 `n` × 分组倍率。
- 某个档位未设置价格时，会使用该模型的基础固定价格。
- 如果没有基础固定价格，则必须完整配置三个档位，避免意外转入按 Token 计费。
- 未配置任何分档固定价时，会使用官方内置的 GPT Image token 表达式计费。

### 使用日志

新版日志详情会展示客户端请求的 `size`、计算后的分辨率档位、`quality`、图片数量以及实际每张单价，便于核对路由和费用。

### 基本配置

1. 在渠道支持模型中加入需要承接的模型，例如 `gpt-image-2`、`gpt-image-2.5-flare`、`gpt-image-2.5-sunburst`。
2. 编辑渠道，在对应的分辨率路由区域填写 1K、2K、4K 上游模型。
3. 在系统模型定价中编辑对应模型，设置基础固定价格和各分辨率档位价格。

克隆本分支：

```bash
git clone https://github.com/a765616527/new-api.git
cd new-api
```
...
# 上游原版仓库： https://github.com/QuantumNous/new-api
...
| `RELAY_IDLE_CONN_TIMEOUT` | Relay HTTP 客户端的空闲 keep-alive 超时时间（秒）；默认沿用 Go 标准库行为，设为 `0` 可禁用 | `90` |
| `RELAY_RESPONSE_HEADER_TIMEOUT` | 等待上游响应头的最长时间（秒）；设为 `0` 可禁用。仅限制响应头等待，不影响收到响应头后的流式传输 | `1800` |
