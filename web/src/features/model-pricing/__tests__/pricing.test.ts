...
describe('shared model pricing', () => {
  ...
  it('persists GPT Image 2 resolution prices with the shared editor path', () => {
    const options = pricingOptions({
      ModelPrice:
        '{"gpt-image-2":0.03,"gpt-image-2@1k":0.01,"gpt-image-2@2k":0.02}',
    })
    const after = applyPricingDraft(options, {
      name: 'gpt-image-2',
      billingMode: 'per-request',
      price: '0.04',
      gptImage2Price1K: '0.011',
      gptImage2Price2K: '',
      gptImage2Price4K: '0.044',
    })

    expect(JSON.parse(after.ModelPrice)).toEqual({
      'gpt-image-2': 0.04,
      'gpt-image-2@1k': 0.011,
